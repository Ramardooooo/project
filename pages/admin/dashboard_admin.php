...
<div class="bg-green-800 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-300 p-6 border border-green-700">
    <div class="flex items-center">
        <div class="p-4 rounded-full bg-<?= $s[3] ?>-600 text-white shadow-md">
            <i class="fas fa-<?= $s[2] ?> text-2xl"></i>
        </div>
        <div class="ml-4">
            <p class="text-sm font-medium text-green-200 uppercase tracking-wide"><?= $s[0] ?></p>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-10">
    <div class="bg-green-800 rounded-2xl shadow-lg p-6 border border-green-700">
        ...
        <div class="space-y-4">
            ...
            while ($user = mysqli_fetch_assoc($recent_users)) {
                echo "<div class='flex items-center justify-between p-3 bg-green-600 rounded-lg hover:bg-green-500 transition-colors'>";
                echo "<span class='font-medium text-green-100'>User baru: {$user['username']}</span>";
                echo "<span class='text-sm text-green-200'>" . date('d/m/Y H:i', strtotime($user['created_at'])) . "</span>";
                echo "</div>";
            }
            ?>
        </div>
    ...
    <div class="bg-green-800 rounded-2xl shadow-lg p-6 border border-green-700">
        ...
        <div class="space-y-4">
            ...
            <div class="p-4 bg-green-700 border-l-4 border-green-400 text-green-100 rounded-r-lg">
...
<div class="bg-green-700 rounded-2xl shadow-lg p-6 mb-10 border border-green-600">
    <h4 class="text-xl font-bold mb-4 text-white">Ringkasan Data</h4>
    <ul class="space-y-3">
        <li class="flex justify-between items-center p-2 bg-green-700 rounded-lg">
            <span class="font-medium text-green-100">Total Warga Aktif:</span>
            <span class="font-bold text-white"><?php echo $data['warga']; ?></span>
        </li>
        <li class="flex justify-between items-center p-2 bg-green-700 rounded-lg">
            <span class="font-medium text-green-100">Total KK Terdaftar:</span>
            <span class="font-bold text-white"><?php echo $data['kk']; ?></span>
        </li>
        <li class="flex justify-between items-center p-2 bg-green-700 rounded-lg">
            <span class="font-medium text-green-100">Total RT:</span>
            <span class="font-bold text-white"><?php echo $data['rt']; ?></span>
        </li>
        <li class="flex justify-between items-center p-2 bg-green-700 rounded-lg">
            <span class="font-medium text-green-100">Total RW:</span>
            <span class="font-bold text-white"><?php echo $data['rw']; ?></span>
        </li>
    </ul>
</div>

<div class="bg-green-800 rounded-2xl shadow-lg p-6 mb-10 border border-green-700">
    <h3 class="text-xl font-bold mb-6 text-white">Kelola Data</h3>
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <a href="/PROJECT/manage_users" class="group p-6 bg-gradient-to-br from-green-800 to-green-700 rounded-xl hover:from-green-700 hover:to-green-600 transition-all duration-300 border border-green-700 hover:border-green-600 hover:shadow-lg">
            <i class="fas fa-users text-green-200 text-3xl mb-3 group-hover:scale-110 transition-transform"></i>
            <h4 class="font-bold text-white mb-2">Kelola User</h4>
            <p class="text-sm text-green-100">Tambah, edit, hapus user</p>
        </a>
        <a href="/PROJECT/manage_rt_rw" class="group p-6 bg-gradient-to-br from-green-800 to-green-700 rounded-xl hover:from-green-700 hover:to-green-600 transition-all duration-300 border border-green-700 hover:border-green-600 hover:shadow-lg">
            <i class="fas fa-map-marker-alt text-green-200 text-3xl mb-3 group-hover:scale-110 transition-transform"></i>
            <h4 class="font-bold text-white mb-2">Kelola RT/RW</h4>
            <p class="text-sm text-green-100">Atur struktur RT dan RW</p>
        </a>
        <a href="manage_master_data.php" class="group p-6 bg-gradient-to-br from-green-800 to-green-700 rounded-xl hover:from-green-700 hover:to-green-600 transition-all duration-300 border border-green-700 hover:border-green-600 hover:shadow-lg">
            <i class="fas fa-database text-green-200 text-3xl mb-3 group-hover:scale-110 transition-transform"></i>
            <h4 class="font-bold text-white mb-2">Data Master</h4>
            <p class="text-sm text-green-100">Kelola data utama sistem</p>
        </a>
    </div>
</div>
